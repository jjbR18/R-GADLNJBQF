...
Imported['GetInformation'] = 1.01;

if (!Imported.CommonPopupCore) {
	console.error('CommonPopupCoreを導入してください。')
}
/*:
 * @plugindesc アイテムの入手などにスライドアニメするインフォメーションを追加するプラグインです。
 * @author Yana
 * 
...
(function() {
	...
	Game_Actor.prototype.changeExp = function(exp, show) {
		var tExp = exp - this._exp[this._classId];
	...
	CommonPopupManager.showInfo = function(object, value, type, actor) {
		var text1 = null;
		if (value === 0) { return }
		switch(type) {
		case 'gold':
		...
		if (actor) {
			text1 = text1.replace(/_actor/g, actor)
		};
